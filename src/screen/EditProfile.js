...
const EditProfile = ({ navigation }) => {
    ...
    const onMobileNumberText = mobileNumber => {
        ...
        if (mobileNumber.length < 10 && !mobileNumber == '') setMobileNumberError(true);
    ...
    const updateProfile = () => {
        if (accountData?.usr_email && name && mobileNumber) {
            ...
            sendRequest({
                ...
                usr_parmentAdrss: "",
                usr_currentAdrss: ""
            }, EndPoints.editUserProfile, 'POST')
                .then(response => {
                    setLoading(false);
                    if (response.status === true) {
                        console.log(response)
                        dispatch(getAccountImfo({ ...accountData, usr_firstName: name, usr_phone: mobileNumber }))
                    } else {
